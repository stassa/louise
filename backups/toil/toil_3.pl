...
% Uses the Prolog engine and avoids using the dynamic db too much.
	...
	,G = (debug(top_program,'Constructing Top program...',[])
	     ,generalise(Pos,MS,Ss_Gen)
	     ,debug_clauses(top_program_g,'Generalised Top program',Ss_Gen)
...
specialise(Ss_Pos,Neg,Ss_Neg):-
	...
	,findall(H-M
		,(member(H-M,Ss_Pos)
		 ,nb_setarg(1,C_C,0)
		 ,debug_clauses(specialise,'Ground metasubstitution atom:',H)
		 ,\+((member(En,Neg)
...
%        debug(prove_,'Proving literal with known metasubs: ~w',[L])
        debug(prove,'Proving literal ~w with known metasubs: ~w',[L,Subs])
