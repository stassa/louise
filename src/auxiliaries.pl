...
%!	experiment_data(+Targets,-Pos,-Neg,-BK,-MS,+Sorted) is det.
%
%	Collect experiment file data for one or more learning Targets.
%
...
%	examples are sorted.
%
%	Sorted is a boolean indicating whether Pos and Neg, the lists of
%	positive and negative examples respectively, should be sorted
%	with sort/2 so that they are in the standard order of terms and
%	contain only unique elements, or not. If Sorted is "false", Pos
%	and Neg are sorted by sort/2. Otherwise, they are collected in
%	the order in which they are generated by the positive and
%	negative examples generators in the current experiment file.
%
%	Use this predicate to collect the elements of a MIL problem for
%	dynamic learning. Use experiment_data/5 to collect the elements
%	of a MIL problem for learn/5.
%
%	@tbd This predicate is necessary because of the sensitivity of
%	dynamic learning to the ordering of positive examples in
%	particular. Other operations in Louise may expect a sorted list
%	of examples.
%
