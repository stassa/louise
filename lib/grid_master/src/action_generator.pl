...
generate_actions(map(Id,Dims,Ms),Vs):-
        ...
        ,findall(M
                ,(member(X/Y,Ts)
                 ,step(R,map(Id,Dims,Ms),X/Y,M)
                 )
                ,Vs_)
        ,sort(Vs_,Vs).


%!      step(+Representation,+Map,+Coordinates,-Move) is det.
%
%       Generate a single Move action to the given Coordinates.
%
...
%
%       Map is a map/3 term for the current grid world map.
%
...
%
step(R,map(Id,Dims,Ms),X/Y,M):-
        step_up(R,[map(Id,Dims,Ms),X/Y],M).
step(R,map(Id,Dims,Ms),X/Y,M):-
        step_right(R,[map(Id,Dims,Ms),X/Y],M).
step(R,map(Id,Dims,Ms),X/Y,M):-
        step_down(R,[map(Id,Dims,Ms),X/Y],M).
step(R,map(Id,Dims,Ms),X/Y,M):-
        step_left(R,[map(Id,Dims,Ms),X/Y],M).
