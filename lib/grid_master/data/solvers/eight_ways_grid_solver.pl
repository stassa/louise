...
Configs:
==
% Started server at http://localhost:59422/pldoc/
Global stack limit 1,073,741,824
Table space 2,147,483,648
Loaded actions in controller_sequences representation.
...
?- list_problem_statistics(s/2).
Positive examples:    1
Negative examples:    0
Background knowledge: 8 [step_down/2,step_left/2,step_right/2,step_up/2,step_up_right/2,step_down_right/2,step_down_left/2,step_up_left/2]
Metarules:            2 [identity,tailrec]
true.

?- list_problem_element(s/2,pos).
s([zero,A/B,C,D,[E,F],[G,H],[I,J],[K,L]],[zero,M/N,O,P,[],[],[],[]]).
true.
==
...
==
?- grid_solver:list_encapsulated_with_primitives(zero).
Positive examples
-----------------
...
----------------------------------
step_down/2:
m(step_down,[zero,0/1,f,A,[A|B],[uppu|C],[step_down|D],[q2|E]],[zero,0/0,f,q2,B,C,D,E]).
m(step_down,[zero,1/1,f,A,[A|B],[uupp|C],[step_down|D],[q2|E]],[zero,1/0,f,q2,B,C,D,E]).

step_left/2:
m(step_left,[zero,1/0,f,A,[A|B],[puup|C],[step_left|D],[q3|E]],[zero,0/0,f,q3,B,C,D,E]).
m(step_left,[zero,1/1,f,A,[A|B],[uupp|C],[step_left|D],[q3|E]],[zero,0/1,f,q3,B,C,D,E]).

step_right/2:
m(step_right,[zero,0/0,f,A,[A|B],[ppuu|C],[step_right|D],[q1|E]],[zero,1/0,f,q1,B,C,D,E]).
m(step_right,[zero,0/1,f,A,[A|B],[uppu|C],[step_right|D],[q1|E]],[zero,1/1,f,q1,B,C,D,E]).

step_up/2:
m(step_up,[zero,0/0,f,A,[A|B],[ppuu|C],[step_up|D],[q0|E]],[zero,0/1,f,q0,B,C,D,E]).
m(step_up,[zero,1/0,f,A,[A|B],[puup|C],[step_up|D],[q0|E]],[zero,1/1,f,q0,B,C,D,E]).

step_up_right/2:
m(step_up_right,[zero,0/0,f,A,[A|B],[ppuu|C],[step_up_right|D],[q01|E]],[zero,1/1,f,q01,B,C,D,E]).

step_down_right/2:
m(step_down_right,[zero,0/1,f,A,[A|B],[uppu|C],[step_down_right|D],[q12|E]],[zero,1/0,f,q12,B,C,D,E]).

step_down_left/2:
m(step_down_left,[zero,1/1,f,A,[A|B],[uupp|C],[step_down_left|D],[q23|E]],[zero,0/0,f,q23,B,C,D,E]).

step_up_left/2:
m(step_up_left,[zero,1/0,f,A,[A|B],[puup|C],[step_up_left|D],[q30|E]],[zero,0/1,f,q30,B,C,D,E]).
...
Learning:
==
?- grid_solver:learn_with_primitives(zero).
% 72,500 inferences, 0.000 CPU in 0.012 seconds (0% CPU, Infinite Lips)
s(A,B):-step_down(A,B).
s(A,B):-step_down_left(A,B).
s(A,B):-step_down_right(A,B).
s(A,B):-step_left(A,B).
s(A,B):-step_right(A,B).
s(A,B):-step_up(A,B).
s(A,B):-step_up_left(A,B).
s(A,B):-step_up_right(A,B).
s(A,B):-step_down(A,C),s(C,B).
s(A,B):-step_down_left(A,C),s(C,B).
s(A,B):-step_down_right(A,C),s(C,B).
s(A,B):-step_left(A,C),s(C,B).
s(A,B):-step_right(A,C),s(C,B).
s(A,B):-step_up(A,C),s(C,B).
s(A,B):-step_up_left(A,C),s(C,B).
s(A,B):-step_up_right(A,C),s(C,B).
Clauses: 16
true.
==
